...
// callback TW URL (/tvii/getAccessTokenTW)
...
app.get("/tvii/linkSocials", (req, res) => {
    res.sendFile("/tviiLinkSocials.html", { root: "./views" });
});

app.get("/tvii/twitterAuth", (req, res) => {
    res.sendFile("/authenticateTwitter.html", { root: "./views" });
});

app.get("/tvii/tumblrAuth", (req, res) => {
    res.sendFile("/authenticateTumblr.html", { root: "./views" });
});

//Generate the code for TVii client and store it here
app.get("/tvii/generateTWCode", (res, req) => {
    try {
        ...
        const signingKey = `${encodeURIComponent(TWConsumerSecret)}&`; // Signing key (consumer secret + empty token secret)
        ...
        const response = fetch("https://api.twitter.com/oauth/request_token", {
            method: "POST",
            headers: {
...
app.get('/tvii/checkForTWRedirect', (req, res) => {
    try {
        ...
        if (!code || !oauthAssociations[code] || oauthAssociations[code].type != "TW") {
            app.sendFile("/invalidAuthCode.html", { root: "./views" });
            return;
        }
...
app.get("/tvii/getAccessTokenTW", async (req, res) => {
    try {
        const { code, oauth_token, oauth_verifier } = req.query;

        if (!code || !oauthAssociations[code] || oauthAssociations[code].oauthToken !== oauth_token || oauthAssociations[code].type !== "TW") {
        ...
        if (responseParams.has('oauth_token')) {
            ...
            return res.sendFile("./views/successTwitterAuth.html");
        } else {
            return res.sendFile("./views/errorTwitterAuth.html");
        }
    } catch (error) {
        console.error("Error in /twttrCodeFinalVerification:", error);
        return res.sendFile("./views/errors/404.html");
    }
});
...
app.get('/tvii/clientCheckTWCodeVerified', (req, res) => {
    try {
        ...
        if (!code || !oauthAssociations[code] || oauthAssociations[code].type !== "TW") {
            return res.status(400).json({ error: "Code is not verified or does not exist." });
        }
